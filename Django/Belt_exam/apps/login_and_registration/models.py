...
class UserManager(models.Manager):
    ...
    def login_validator(self, postData):
        errors = {}
        # email = User.objects.get(email=postData["email"])
        # password = bcrypt.hashpw(postData["password"].encode(), email.password.encode())
        ...
        if not self.filter(email=postData["email"]):
            errors['email'] = "Invalid information. Please enter valid information"
        if len(postData["password"]) < 8:
            errors['password'] = "Invalid information. Please enter valid information"
